...
import whisper  # Используем официальный пакет whisper вместо faster-whisper
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "server:app",  # Укажите путь к приложению в формате "файл:app"
        host="0.0.0.0",
        port=8000,
        workers=1,  # Для GPU лучше использовать один воркер
        loop="asyncio",
        reload=True  # Отключите автоперезагрузку для продакшена
    )